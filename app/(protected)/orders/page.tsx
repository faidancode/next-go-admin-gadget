      ...
      <div className="container pt-2 space-y-4 mt-4">
        ...
        >
          ...
          <div className="flex flex-col gap-4 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="w-full overflow-x-auto no-scrollbar sm:w-auto sm:overflow-visible py-1">
              <TabsList className="flex bg-transparent h-auto p-0 gap-1 justify-start min-w-max border-none shadow-none">
                {STATUSES.map((st) => (
                  <TabsTrigger
                    key={st}
                    value={st}
                    className={cn(
                      "px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300",
                      "data-[state=active]:bg-slate-900 data-[state=active]:text-white data-[state=active]:shadow-md data-[state=active]:shadow-slate-200",
                      "data-[state=inactive]:text-slate-400 data-[state=inactive]:hover:text-slate-600 data-[state=inactive]:hover:bg-slate-100",
                      "focus-visible:ring-0 hover:cursor-pointer"
                    )}
                  >
                    {st}
                  </TabsTrigger>
